...
const nav = [
  ...
  ["Bible & Faith", "/resources/kids-learning#scripture"],
  ["Shop", "/shop"],
  ["About", "/about"],
] as const;
  ...
  return (
    <header className="site-header">
      ...
      <div className="header-actions">
        <a className="button small" href="/book">Book a Service</a>
      ...
      {open && <div className="mobile-menu"><nav aria-label="Mobile navigation">{nav.map(([label,href])=><a key={label} href={href} onClick={()=>setOpen(false)}>{label}</a>)}</nav><a className="button gold" href="/book" onClick={()=>setOpen(false)}>Book / request a service</a></div>}
    </header>
  );
}
