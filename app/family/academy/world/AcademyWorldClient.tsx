...
export function AcademyWorldClient({ learnerName = "Learner", roleLabel = "Family Academy", mission }: { learnerName?: string; roleLabel?: string; mission: AcademyWorldMission }) {
  ...
  const zones = useMemo<Zone[]>(() => {
    ...
    return [
      { id: "scripture", title: "Scripture Hall", subtitle: "Word · Wisdom · Truth", status: "current", x: 13, y: 16 },
      ...
      { id: "garden", title: "Garden Courtyard", subtitle: "Stewardship · Creation · Health", status: level >= 3 ? "open" : "locked", x: 14, y: 52, requirement: "Advance through your pathway" },
      { id: "kitchen", title: "Kitchen Lab", subtitle: "Nourish · Create · Provide", status: level >= 4 ? "open" : "locked", x: 87, y: 16, requirement: "Advance through your pathway" },
      { id: "creative", title: "Creative Studio", subtitle: "Imagine · Design · Express", status: level >= 5 ? "open" : "locked", x: 87, y: 35, requirement: "Advance through your pathway" },
      { id: "technology", title: "Technology Lab", subtitle: "Innovate · Build · Solve", status: level >= 6 ? "open" : "locked", x: 87, y: 55, requirement: "Reach the required role and age stage" },
    ];
  }, [mission.levelNumber]);
  ...
  const move = (dx: number, dy: number) => {
    setAvatar((current) => ({ x: clamp(current.x + dx, 5, 95), y: clamp(current.y + dy, 12, 86) }));
  ...
  return <div className={styles.shell}>
    <section className={styles.gameFrame} ref={viewportRef} aria-label="Hands Gifted Kingdom Academy">
      <div className={styles.kingdomCanvas}>
        <img className={styles.sceneArtwork} src={KINGDOM_BACKGROUND} alt="" aria-hidden="true" />

        {zones.map((zone) => (
          <button
            key={zone.id}
            className={`${styles.destination} ${zone.status === "locked" ? styles.destinationLocked : ""} ${nearest.zone.id === zone.id && canInteract ? styles.destinationNear : ""}`}
            style={{ left: `${zone.x}%`, top: `${zone.y}%` }}
            onClick={() => {
              setAvatar({ x: zone.x, y: clamp(zone.y + 8, 12, 86) });
          ...
          >
            <span className={styles.srOnly}>{zone.title} — {zone.subtitle}</span>
            {zone.status === "locked" ? <span className={styles.lockBadge}>🔒</span> : null}
          </button>
        ))}

        <div className={styles.locator} style={{ left: `${avatar.x}%`, top: `${avatar.y}%` }} aria-hidden="true" />
      </div>

      <div className={styles.profileHud}>
        <div className={styles.profileOrb}>♛</div>
        <div><strong>{learnerName}</strong><span>{roleLabel}</span><small>Level {mission.levelNumber}</small></div>
      </div>
      <div className={styles.utilityHud}><span>Map</span><span>Rewards</span><span>Journal</span><span>Settings</span></div>
      <div className={styles.missionHud}><span>♛ CURRENT MISSION</span><strong>{mission.assignmentTitle}</strong><small>{canInteract ? (nearest.zone.status === "locked" ? `${nearest.zone.title} is locked` : `Visit ${nearest.zone.title}`) : "Explore the Kingdom"}</small></div>
    ...
    {showMission && selected.status !== "locked" ? <section className={styles.lessonOverlay}>
      ...
      {selected.id === "scripture" ? <>
        ...
        <div className={styles.questBoard}><h3>{mission.assignmentTitle}</h3><p>{mission.assignmentInstructions}</p><ol><li>Read the mission carefully.</li><li>Open your KJV 1611 and Apocrypha when the mission calls for Scripture.</li><li>Use your study notebook to write discoveries, questions, and what the lesson means for your role.</li><li>Use web research only when the mission permits it, and keep outside research separate from what the Scripture itself says.</li><li>When you feel ready, take the Level Test.</li></ol></div>
        <div className={styles.lessonActions}><button>Need a Hint</button><button>Study Notes</button>{mission.assessment?.questions.length ? <button onClick={() => setShowTest((value) => !value)}>{showTest ? "Close Test" : "Take Level Test"}</button> : <button disabled>Test Being Prepared</button>}</div>
