...
export function AcademyWorldClient({ learnerName = "Learner", roleLabel = "Family Academy", mission }: { learnerName?: string; roleLabel?: string; mission: AcademyWorldMission }) {
  ...
  const [avatar, setAvatar] = useState({ x: 49, y: 78 });
  ...
  const zones = useMemo<Zone[]>(() => {
    ...
    return [
      { id: "scripture", title: "Scripture Hall", subtitle: "Bible study, leaders, law and understanding", icon: "📖", status: "current", x: 50, y: 23 },
      { id: "family", title: "Family Hall", subtitle: "Love, communication, bonding and restoration", icon: "🏠", status: level >= 2 ? "open" : "locked", x: 22, y: 43, requirement: "Complete the current Academy level" },
      { id: "garden", title: "Garden Lab", subtitle: "Nature, science, stewardship and growing food", icon: "🌿", status: level >= 3 ? "open" : "locked", x: 79, y: 43, requirement: "Advance through your pathway" },
      { id: "kitchen", title: "Kitchen Lab", subtitle: "Cooking, measurement, nutrition and service", icon: "🍲", status: level >= 4 ? "open" : "locked", x: 18, y: 70, requirement: "Advance through your pathway" },
      { id: "creative", title: "Creative Studio", subtitle: "Art, sewing, braiding, music and making", icon: "🎨", status: level >= 5 ? "open" : "locked", x: 80, y: 70, requirement: "Advance through your pathway" },
      { id: "technology", title: "Technology Lab", subtitle: "Digital skills, building and game creation", icon: "💻", status: level >= 6 ? "open" : "locked", x: 50, y: 88, requirement: "Reach the required role and age stage" },
    ];
  }, [mission.levelNumber]);
  ...
  const move = (dx: number, dy: number) => {
    setAvatar((current) => ({ x: clamp(current.x + dx, 8, 92), y: clamp(current.y + dy, 16, 91) }));
  ...
  return <div className={styles.shell}>
    <header className={styles.hud}>
      <div><span className={styles.eyebrow}>{roleLabel}</span><h1>{learnerName}&apos;s Academy</h1></div>
      <div className={styles.progressCard}><strong>Level {mission.levelNumber}</strong><span>{mission.levelName}</span><small>{unlockedCount} areas discovered</small>{mission.bestScore !== null ? <small>Best score: {mission.bestScore}%</small> : null}</div>
    </header>

    <section className={styles.gameFrame} aria-label="Hands Gifted explorable Academy world">
      <div className={styles.skyGlow} />
      <div className={styles.backWall}><div className={styles.banner}>HANDS GIFTED FAMILY ACADEMY</div></div>
      <div className={styles.leftWall} /><div className={styles.rightWall} />
      <div className={styles.floorGrid} />
      <div className={styles.gardenBed}><span>🌱</span><span>🌿</span><span>🌾</span><span>🌻</span></div>
      ...
      {zones.map((zone) => <button key={zone.id} className={`${styles.portal} ${zone.status === "locked" ? styles.portalLocked : ""} ${nearest.zone.id === zone.id && canInteract ? styles.portalNear : ""}`} style={{ left: `${zone.x}%`, top: `${zone.y}%` }} onClick={() => { setAvatar({ x: zone.x, y: clamp(zone.y + 9, 16, 91) }); setSelectedId(zone.id); setShowMission(false); setShowTest(false); }} aria-label={`${zone.title} ${zone.status}`}>
        <span className={styles.portalDoor}>{zone.status === "locked" ? "🔒" : zone.icon}</span>
        <strong>{zone.title}</strong>
        <small>{zone.status === "current" ? "CURRENT" : zone.status === "open" ? "OPEN" : "LOCKED"}</small>
      </button>)}

      <div className={styles.avatar} style={{ left: `${avatar.x}%`, top: `${avatar.y}%` }} aria-label={`${learnerName} avatar`}>
        <div className={styles.avatarHead}>●</div><div className={styles.avatarBody}>▲</div><small>{learnerName}</small>
      </div>

      <div className={styles.gameHudLeft}><span>MISSION</span><strong>{mission.assignmentTitle}</strong></div>
      <div className={styles.gameHudRight}><span>WASD / ARROWS</span><strong>Walk & Explore</strong></div>
      {canInteract ? <button className={styles.interactPrompt} onClick={interact}>{nearest.zone.status === "locked" ? `🔒 ${nearest.zone.title}` : `E · Enter ${nearest.zone.title}`}</button> : <div className={styles.interactPromptMuted}>Walk closer to a doorway</div>}

      <div className={styles.mobilePad} aria-label="movement controls">
        <button onClick={() => move(0, -4)}>▲</button><div><button onClick={() => move(-4, 0)}>◀</button><button onClick={interact}>●</button><button onClick={() => move(4, 0)}>▶</button></div><button onClick={() => move(0, 4)}>▼</button>
      </div>
    </section>
