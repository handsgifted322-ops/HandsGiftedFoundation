...
export default async function FamilyDashboardPage(){
  ...
  return <main><SiteHeader/><section className="inner-hero"><span>PRIVATE FAMILY DASHBOARD</span><h1>{state==="child"?`${name}'s Dashboard`:"Family Dashboard"}</h1><p>Private household information is available only to the authenticated family member it belongs to.</p></section><section className="section">
    ...
    state==="parent"?<div className="access-note"><strong>Parent account detected.</strong><p>{name}, household management belongs in the protected Parent Command Center.</p><p><a className="button" href="/command-center">Open Parent Command Center</a> <a className="button" href="/command-center/sops">Manage SOP Resources</a> <a className="button" href="/family/academy/world">Enter My Academy</a></p></div>:
    ...
    <><div className="section-heading left no-margin"><span>{ageGroup.replace("_"," ")} workspace</span><h2>{name}'s active responsibilities</h2><p>Your signed-in account is connected to your own assignments and current chore guides.</p><p><a className="button" href="/family/academy/world">Enter My Academy</a> <a className="button" href="/family/academy/resources">Open Practical Life Resources</a></p></div>{assignments.length?<div className="detail-grid" style={{marginTop:32}}>{assignments.map(a=><article key={a.id}><span>{a.assignment_type}</span><h3>{a.title}</h3><p><strong>Status:</strong> {a.status.replaceAll("_"," ")} · <strong>Mom Check:</strong> {a.parent_check_status.replaceAll("_"," ")}</p>{a.instructions?<p><strong>Instructions:</strong> {a.instructions}</p>:null}{a.notes?<p><strong>Notes / Scripture:</strong> {a.notes}</p>:null}</article>)}</div>:<div className="access-note" style={{marginTop:32}}><strong>No active assignments are showing yet.</strong><p>Your dashboard is connected; new approved responsibilities will appear here.</p></div>}<div className="section-heading left" style={{marginTop:48}}><span>Exact chore guides</span><h2>Current rotation instructions</h2><p>Need the full age-appropriate procedure? Open Practical Life Resources to study the same live SOPs before completing your responsibility.</p></div>{rotations.length?<div className="detail-grid">{rotations.map(r=>{const z=zoneById.get(r.zone_id);const s=sopByZone.get(r.zone_id);return <article key={r.zone_id}><span>{z?.name??"Chore"}</span><h3>{s?.title??"Instructions"}</h3><p><strong>Rotation review:</strong> {r.next_rotation_on}</p>{z?.daily_standard?<p><strong>Daily standard:</strong> {z.daily_standard}</p>:null}{z?.age_guidance?<p><strong>Age guidance:</strong> {z.age_guidance}</p>:null}{s?.safety_notes?<p><strong>Safety:</strong> {s.safety_notes}</p>:null}{s?.scripture_reference?<p><strong>Scripture:</strong> {s.scripture_reference}</p>:null}</article>})}</div>:<p>No current rotation is assigned to this account.</p>}</>}
